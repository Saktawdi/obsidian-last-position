...
- [ ] **修复 retry count 的语义并增加可观测性（关联 #10）**
  - 将“次数”配置替换或补充为恢复超时时间、重试间隔与最大尝试次数；避免把 150 次尝试在约 1 秒内耗尽。
  - 只在恢复超时后提示，并在日志中记录文件、leaf、目标位置、实际位置和终止原因。
  - 验收：增加最大次数不会因固定短间隔而无效；长文档或延迟渲染的文件可在配置窗口内恢复。
