...
- [x] **提供安全的数据迁移与导入校验**
  ...
  - 评估结果：已完成。v2 状态缺失书签字段时归一化为空映射；版本化导出包含书签；旧版数组/文件映射导入不创建书签且合并时保留现有书签；导入严格校验 JSON、路径、位置、时间戳和书签记录。
...
- [x] **评估并实现位置书签（关联 #4）**
  - 已定义按文件共享的书签数据结构；提供 `Last Position: Save Bookmark` / `Last Position: Select Bookmark`（中文环境显示对应中文名称）命令，可命名保存、搜索选择并跳转到历史高度。
  - 重名自动生成 `(1)`、`(2)` 后缀；选择跳转经过活动文件校验，不会覆盖普通 last-position 历史。
  - 新增 `Last Position: Remove Bookmark` 命令（中文环境显示“删除书签”）：按当前文件选择书签，二次确认后删除并持久化；编辑书签和基于块/标题的抗漂移定位仍后续评估。
  - 来源：[issue #4](https://github.com/Saktawdi/obsidian-last-position/issues/4)。
